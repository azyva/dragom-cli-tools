...
rem Dragom execution environment. See the comment in set-env.sh for more
rem information.
rem
...
rem ce qui n'est pas possible dans la commande for /f.
...
set /p PROPERTIES= < %TEMP_FILE%
del %TEMP_FILE%

for /f "tokens=1* delims==" %%P in ("%PROPERTIES%") do (
  set PROPERTY=%%P
...
rem   into the posittional variables %1, %2, etc.
